...
rcParams['font.sans-serif'] = ['Liberation']
...
class Radar:
    ...
    def __init__(
            self, background_color="#FFFFFF", patch_color="#D6D6D6", fontfamily="DejaVuSerif",
            label_fontsize=13, range_fontsize=9, label_color="#000000", range_color="#000000"
    ):
        """
    ...
    def plot_radar(self, ranges, params, values, radar_color, filename=None, dpi=300,
                   title=dict(), alphas=[0.1, 0.1], compare=False, endnote=None,
                   end_size=7, end_color="#95919B", image_1=None, image_coord_1=None, image_2=None, image_coord_2=None,
                   figax=None, logo=None, logo_coord=None, **kwargs):
        """
        ...
        ## add explanations
        ax.text(-22, -23.5,
                "Le cercle intérieur représente le 1er centile\nLe cercle extérieur représente le 99ème centile",
                fontfamily=self.fontfamily, ha='left',
        ...
        ## add image
        if image_1 != None and image_coord_1 != None:
            fig = add_image(image_1, fig, image_coord_1[0], image_coord_1[1], image_coord_1[2], image_coord_1[3],
                            **kwargs)

        if image_2 != None and image_coord_2 != None:
            fig = add_image(image_2, fig, image_coord_2[0], image_coord_2[1], image_coord_2[2], image_coord_2[3],
                            **kwargs)

        if logo != None and logo_coord != None:
            fig = add_image(logo, fig, logo_coord[0], logo_coord[1], logo_coord[2], logo_coord[3], **kwargs)
    ...
    def __plot_titles(self, ax, title):
        ...
        # ICI CHANGER LA POSITION DU TITRE
        if title.get('title_name') and title.get('title_name_2') == None:
            ## plot the title name
            ax.text(-11, 24, title['title_name'], fontsize=title['title_fontsize'], fontweight='bold',
                    fontdict={'color': title['title_color']}, fontfamily=self.fontfamily)

        if title.get('subtitle_name') and title.get('title_name_2') == None:
            ## plot the title name
            ax.text(-7, 22, title['subtitle_name'], fontsize=title['subtitle_fontsize'],
        ...
        if title.get('title_name_2'):
            ...
            ax.text(-18.2, 24, title['title_name'], fontsize=title['title_fontsize'], fontweight='bold',
                    fontdict={'color': title['title_color']}, fontfamily=self.fontfamily)
            ax.text(17.2, 24, title['title_name_2'], fontsize=title['title_fontsize_2'], fontweight='bold',
        ...
        if title.get('subtitle_name_2'):
            ...
            ax.text(-18.2, 21.5, title['subtitle_name'], fontsize=title['subtitle_fontsize'],
                    fontdict={'color': title['subtitle_color']}, fontfamily=self.fontfamily)
            ax.text(17.2, 21.5, title['subtitle_name_2'], fontsize=title['subtitle_fontsize_2'],
    ...
    def __add_ranges(self, ranges, ax):
        ...
        radius = [4.9, 6.9, 8.9, 10.9, 12.9, 14.9, 17]
        ...
        for rng in ranges:
            value = np.linspace(start=rng[0], stop=rng[1], num=7)
            range_values = np.append(range_values, value)

        range_values = range_values.reshape((len(ranges), 7))
        ...
        for i in range(len(range_values)):
            ...
            if range_list[0] > range_list[-1]:
                ...
                if values[i] >= range_list[0]:
                    ...
                    x_coord, y_coord = get_indices_between(range_list=range_list, coord_list=coord_list,
